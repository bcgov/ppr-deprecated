...
const debtorParties = [firstSecuredParty]

describe('DebtorParties.vue', (): void => {
  describe(':props', (): void => {

    // General form tests.
    it(':editing - false, add button does not exist', (): void => {
      const properties = ref({ editing: false, value: debtorParties })
  ...
  describe('@events', (): void => {
    it('@input - add button should emit input with new list', async (): Promise<void> => {
      ...
      const button = wrapper.get('[data-test-id="DebtorParties.button.add"]')
